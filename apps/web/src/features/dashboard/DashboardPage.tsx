...
type DashboardWidget = 'operational' | 'attention' | 'pulse' | 'kpis' | 'reservations' | 'conversions' | 'performance' | 'flow' | 'ud' | 'pieces';
const WIDGET_LABELS: Record<DashboardWidget, string> = { operational: 'Inicio operativo', attention: 'Atención del día', pulse: 'Pulso Espartanos', kpis: 'Indicadores principales', reservations: 'Resultados de reservas', conversions: 'Cola de conversiones', performance: 'Meta y Google', flow: 'Ciclo Maestro', ud: 'Unidades de dedicación', pieces: 'Estado de piezas' };
const ROLE_PRESETS: Record<string, DashboardWidget[]> = {
  admin: ['operational', 'attention', 'pulse', 'kpis', 'reservations', 'conversions', 'performance', 'flow', 'ud', 'pieces'],
  operations_director: ['operational', 'attention', 'pulse', 'kpis', 'reservations', 'conversions', 'flow', 'ud', 'pieces'],
  commercial_director: ['operational', 'attention', 'pulse', 'kpis', 'reservations', 'performance', 'flow'],
  community_manager: ['operational', 'attention', 'pulse', 'kpis', 'reservations', 'performance', 'flow', 'pieces'],
  creative_director: ['operational', 'attention', 'pulse', 'kpis', 'flow', 'ud', 'pieces'],
  art_director: ['operational', 'attention', 'kpis', 'flow', 'ud', 'pieces'],
  av_director: ['operational', 'attention', 'kpis', 'flow', 'pieces'],
  designer: ['operational', 'attention', 'kpis', 'pieces'],
  audiovisual: ['operational', 'attention', 'kpis', 'pieces'],
};
/**
 ...
 *
 * Arranca por los dos colores de marca y sigue con tonos que se distinguen entre sí también
  ...
  return (
    <div className="page">
      ...
      {/*
        ...
        por delante obliga a bajar para encontrar lo que está por ocurrir hoy.
      */}
      {widgetVisible('operational') && <OperationalHome />}
      ...
      {!personalView && widgetVisible('pulse') && <PulsoEspartano />}
      ...
      {widgetVisible('pieces') && data.pieces && data.pieces.length > 0 && <div className="section">
        ...
        <div className="dashboard-charts-row">
          ...
          <div className="dashboard-chart-card"><h3>Rendimiento por tipo</h3>
            <ResponsiveContainer width="100%" height={250}><RadarChart data={data.pieces.map((p) => ({ status: statusLabel(p.status), count: p.count, fullMark: Math.max(...data.pieces!.map((x) => x.count)) }))}><PolarGrid /><PolarAngleAxis dataKey="status" tick={{ fontSize: 10 }} /><PolarRadiusAxis tick={{ fontSize: 10 }} /><Radar dataKey="count" stroke="#0ec6b8" fill="#0ec6b8" fillOpacity={0.2} /></RadarChart></ResponsiveContainer>
          </div>
        </div>
      </div>}
