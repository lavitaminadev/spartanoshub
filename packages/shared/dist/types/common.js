"use strict";
/**
 * @fileoverview Common API contracts shared across the Espartanos monorepo.
 *
 * These types define the generic response envelopes and pagination shape used by
 * both the NestJS backend and the React frontend. Keeping them in `@espartanos/shared`
 * guarantees that both sides of the network boundary agree on the wire format.
 */
Object.defineProperty(exports, "__esModule", { value: true });
//# sourceMappingURL=common.js.map