...
export function FlujoSimpleDeEncuesta({
  ...
  origen: string;
  renderPregunta: (pregunta: SurveyQuestion, valor: string | number | undefined, cambiar: (valor: string | number) => void) => JSX.Element;
}): JSX.Element {
  ...
  const restantes = survey.questions.filter((pregunta) => pregunta.id !== preguntaNota.id);
  const faltanObligatorias = restantes.some((pregunta) => pregunta.required && (respuestas[pregunta.id] === undefined || String(respuestas[pregunta.id]).trim() === ''));
  ...
  const iniciar = useMutation({
    ...
    onSuccess: (datos) => {
      ...
      else setPaso(restantes.length > 0 ? 'ofrecer' : 'final');
    },
  });

  const completar = useMutation({
    mutationFn: (datos: { answers?: Answers; teamMessage?: string; responder?: boolean; terminar?: boolean }) => {
  ...
  const cerrar = (datos: { answers?: Answers; teamMessage?: string; responder?: boolean } = {}) => {
    ...
    <div className="flujo-encuesta">
      ...
      {paso === 'mensaje' && <>
        ...
        <div className="flujo-encuesta-acciones">
          <button type="button" className="btn btn-primary btn-block" disabled={!mensaje.trim() || completar.isPending} onClick={() => cerrar({ teamMessage: mensaje })}>
            {completar.isPending ? 'Enviando…' : 'Enviar al equipo'}
          </button>
          <button type="button" className="btn btn-outline btn-block" disabled={completar.isPending} onClick={() => cerrar()}>Prefiero no escribir</button>
        </div>
      </>}
      ...
      {paso === 'preguntas' && <>
        <div className="public-survey-questions">
          {restantes.map((pregunta) => (
            <div key={pregunta.id}>
              {renderPregunta(pregunta, respuestas[pregunta.id], (valor) => setRespuestas((actuales) => ({ ...actuales, [pregunta.id]: valor })))}
            </div>
          ))}
        </div>
        <button type="button" className="btn btn-primary btn-block public-survey-submit" disabled={faltanObligatorias || completar.isPending} onClick={() => cerrar({ answers: respuestas, responder: true })}>
