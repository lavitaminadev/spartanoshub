...
const process_template_defaults_1 = require("./process-template-defaults");
let ProcessTemplatesService = class ProcessTemplatesService {
    ...
    async getSteps(organizationId, code) {
        ...
        return template?.steps?.length ? template.steps : process_template_defaults_1.PROCESS_TEMPLATE_DEFAULTS[code]?.steps ?? [];
    }
    ...
    async reset(code, organizationId) {
        const value = process_template_defaults_1.PROCESS_TEMPLATE_DEFAULTS[code];
        if (!value)
    ...
    async ensureDefaults(organizationId) {
        ...
        const missing = Object.entries(process_template_defaults_1.PROCESS_TEMPLATE_DEFAULTS).filter(([code]) => !existing.has(code)).map(([code, value]) => this.templates.create({ organizationId, code, ...value }));
        if (missing.length)
...
exports.ProcessTemplatesService = ProcessTemplatesService = __decorate([
    ...
    __param(0, (0, typeorm_1.InjectRepository)(process_template_entity_1.ProcessTemplate)),
