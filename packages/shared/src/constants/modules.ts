 ...
 */
export const INITIAL_OPERATION_MODULES = new Set<string>([
  'dashboard',
  'users',
  'clients',
  'crm',
...
export function isModuleInInitialOperationScope(module: string | undefined, role?: string): boolean {
  ...
  if (module === 'clients' || module === 'integrations') return role !== 'client';
  ...
  if (module === 'surveys') return ['admin', 'commercial_director', 'operations_director', 'community_manager', 'client'].includes(role ?? '');
  return true;
}
