...
export function CommandPalette() {
  ...
  const reservationsQuery = useQuery<{ data?: ReservationResult[]; items?: ReservationResult[] }>({ queryKey: ['command-reservations'], queryFn: () => api.get('/reservations?page=1&pageSize=50'), enabled: open && canOpen('/reservations'), retry: false });
  ...
  const items = useMemo<SearchItem[]>(() => {
    ...
    const records: SearchItem[] = [
      ...
      ...(reservationsQuery.data?.data ?? reservationsQuery.data?.items ?? []).map((reservation) => ({ id: `reservation-${reservation.id}`, group: 'Reservas recibidas', title: reservation.guestName, description: `#${reservation.referenceCode} · ${reservation.guestEmail || reservation.guestPhone || reservation.status}`, path: `/reservations?tab=bookings&search=${encodeURIComponent(reservation.referenceCode)}` })),
