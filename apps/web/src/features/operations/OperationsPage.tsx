...
export function OperationsPage() {
  ...
  if (!data) return <EmptyState icon="chart" title="Sin datos" description="No hay información de operaciones disponible en este momento." />;
  ...
  return (
    <div className="page">
      ...
      <div className="card-grid">
        <Card title="Capacidad Total" value={data.totalCapacity > 0 ? `${data.totalCapacity} UD` : 'Sin definir'} icon="chart" color="var(--info)" />
        <Card title="Demanda Actual" value={`${data.usedCapacity} UD`} icon="bolt" color="var(--amber)" />
        <Card title="Utilización" value={data.totalCapacity > 0 ? `${utilizationPct}%` : 'Sin definir'} icon="chart" color={utilizationPct > 80 ? 'var(--danger)' : 'var(--success)'} />
      </div>
      ...
      <div className="section">
        ...
        {data.team.length === 0 ? (
          <EmptyState icon="team" title="Equipo vacío" description="No hay miembros registrados en el equipo todavía." />
        ) : (
