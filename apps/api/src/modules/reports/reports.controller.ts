...
export class ReportingController {
  ...
  async kpi(@Req() req: AuthenticatedRequest) {
    ...
    // Estas cifras son ingresos y cartera agregados. Sin acotar, una dirección asignada a
    // tres cuentas veía la facturación de la agencia entera: no hay datos personales fila a
    // fila, pero sigue siendo información que su alcance no cubre.
    ...
    const [revRow] = await this.dataSource.query(
      `SELECT COALESCE(SUM(total),0) as total FROM invoices WHERE organization_id = ? AND status = 'paid' AND YEAR(created_at) = YEAR(NOW()) ${scopedFilter}`,
      [orgId, ...scoped],
    );
    const [clientRow] = await this.dataSource.query(
      `SELECT COUNT(*) as total FROM clients WHERE organization_id = ? AND status = 'active' ${clientFilter}`,
      [orgId, ...scoped],
    );
    const [udRow] = await this.dataSource.query(
      `SELECT COALESCE(SUM(contracted),0) as total FROM ud_budgets WHERE client_id IN (SELECT id FROM clients WHERE organization_id = ? ${clientFilter})`,
      [orgId, ...scoped],
    );
    const [retRow] = await this.dataSource.query(
      `SELECT COUNT(DISTINCT client_id) * 100.0 / NULLIF((SELECT COUNT(*) FROM clients WHERE organization_id = ? ${clientFilter}), 0) as pct FROM pieces WHERE organization_id = ? AND status = 'delivered' ${scopedFilter}`,
      [orgId, ...scoped, orgId, ...scoped],
    );
