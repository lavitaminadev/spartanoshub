...
exports.OperationalSchema1710000000019 = void 0;
const typeorm_1 = require("typeorm");
class OperationalSchema1710000000019 {
    constructor() {
        this.name = 'OperationalSchema1710000000019';
    }
    async up(queryRunner) {
        ...
        await this.createIfMissing(queryRunner, new typeorm_1.Table({
            name: 'documents',
            columns: [
                this.id(), this.orgId(),
    ...
    async ensureUserClientScope(queryRunner) {
        let users = await queryRunner.getTable('users');
        if (!users?.findColumnByName('client_id')) {
            await queryRunner.addColumn('users', new typeorm_1.TableColumn({ name: 'client_id', type: 'uuid', isNullable: true }));
            users = await queryRunner.getTable('users');
        }
        if (!users?.indices.some((index) => index.name === 'IDX_users_client_id')) {
            await queryRunner.createIndex('users', new typeorm_1.TableIndex({ name: 'IDX_users_client_id', columnNames: ['client_id'] }));
        }
    ...
    async ensureNotifications(queryRunner) {
        ...
        if (!table?.findColumnByName('organization_id')) {
            await queryRunner.addColumn('notifications', new typeorm_1.TableColumn({ name: 'organization_id', type: 'uuid', isNullable: true }));
            ...
            await queryRunner.changeColumn('notifications', 'organization_id', new typeorm_1.TableColumn({ name: 'organization_id', type: 'uuid' }));
            table = await queryRunner.getTable('notifications');
        }
        if (!table?.indices.some((index) => index.name === 'IDX_notifications_user_read')) {
            await queryRunner.createIndex('notifications', new typeorm_1.TableIndex({ name: 'IDX_notifications_user_read', columnNames: ['user_id', 'read', 'created_at'] }));
        }
        if (!table?.indices.some((index) => index.name === 'IDX_notifications_org_user')) {
            await queryRunner.createIndex('notifications', new typeorm_1.TableIndex({ name: 'IDX_notifications_org_user', columnNames: ['organization_id', 'user_id'] }));
        }
    }
    async ensureExistingOperationalColumns(queryRunner) {
        await this.ensureColumn(queryRunner, 'corrections', new typeorm_1.TableColumn({ name: 'billable_extra', type: 'boolean', default: false }));
        await this.ensureColumn(queryRunner, 'corrections', new typeorm_1.TableColumn({ name: 'charge_note_required', type: 'boolean', default: false }));
        await this.ensureColumn(queryRunner, 'approval_requests', new typeorm_1.TableColumn({ name: 'client_id', type: 'uuid', isNullable: true }));
        ...
        await this.ensureColumn(queryRunner, 'meetings', new typeorm_1.TableColumn({ name: 'client_id', type: 'uuid', isNullable: true }));
    ...
    async createCrmTables(queryRunner) {
        await this.createIfMissing(queryRunner, new typeorm_1.Table({
            name: 'crm_contacts',
            columns: [
                this.id(), this.orgId(),
    ...
    async createCatalogAndContractTables(queryRunner) {
        await this.createIfMissing(queryRunner, new typeorm_1.Table({
            name: 'catalog_packs',
            columns: [
                this.id(), this.orgId(),
    ...
    async createAudiovisualTables(queryRunner) {
        await this.createIfMissing(queryRunner, new typeorm_1.Table({
            name: 'moodboards',
            columns: [
    ...
    async createIfMissing(queryRunner, table) {
        if (!(await queryRunner.hasTable(table.name)))
            await queryRunner.createTable(table, true);
    }
    async ensureColumn(queryRunner, tableName, column) {
        const table = await queryRunner.getTable(tableName);
        if (table && !table.findColumnByName(column.name))
            await queryRunner.addColumn(tableName, column);
    }
    async ensureIndex(queryRunner, tableName, name, columnNames) {
        const table = await queryRunner.getTable(tableName);
        if (table && !table.indices.some((index) => index.name === name)) {
            await queryRunner.createIndex(tableName, new typeorm_1.TableIndex({ name, columnNames }));
        }
    }
    async ensureForeignKey(queryRunner, tableName, column, referencedTable, referencedColumn, onDelete) {
        const table = await queryRunner.getTable(tableName);
    ...
    foreignKey(column, table, referencedColumn, onDelete) {
        return new typeorm_1.TableForeignKey({ columnNames: [column], referencedTableName: table, referencedColumnNames: [referencedColumn], onDelete });
    }
}
