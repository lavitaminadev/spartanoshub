...
function explainCloudinaryError(message: string): string {
  ...
  if (lower.includes('unknown api_key')) {
    return 'Cloudinary rechazó la API Key. Usa la API Key numérica del dashboard de Cloudinary, no el correo ni la contraseña de Espartanos.';
  }
...
export function CloudinaryConfigModal({ open, onClose }: CloudinaryConfigModalProps) {
  ...
  const validationMessage = useMemo(() => {
    ...
    if (normalizedApiKey && looksLikeEmail(normalizedApiKey)) return 'La API Key no es un correo. No uses el usuario de Espartanos; usa la API Key numérica de Cloudinary.';
    ...
    if (normalizedApiSecret && looksLikeEmail(normalizedApiSecret)) return 'El API Secret no es un correo ni la contraseña de Espartanos. Copia el API Secret desde Cloudinary.';
    return '';
    ...
    <Modal open={open} onClose={onClose} title="Cloudinary global">
      <div className="cloudinary-config-modal">
        <p className="page-subtitle">
          ...
          Esta configuración aplica a toda la organización y usa credenciales propias de Cloudinary, no las de Espartanos.
        </p>

        <div className={`cloudinary-status ${connected ? 'connected' : 'disconnected'}`}>
          <span>{connected ? '● Conectado' : '○ Desconectado'}</span>
          <small>{isEnvironment ? 'Configurado por variables de entorno' : isConfiguredInApp ? 'Configurado en Espartanos' : 'Sin credenciales guardadas'}</small>
        </div>
        ...
        >
          <label>
            ...
            <input
              ...
              required={!connected}
            />
            <small>Está en Cloudinary Dashboard como “Cloud name”. No es el nombre de usuario de Espartanos.</small>
          </label>
          <label>
          ...
          <label>
            ...
            <small>El API Secret tampoco es la contraseña de Espartanos.</small>
          </label>
