...
export function ReservationLocalHubPage() {
  ...
  return <main className="reservation-local-hub">
    <header className="local-hub-hero">
      ...
      <div className="portal-item-actions"><Link className="btn btn-primary" to={`${base}/forms/${id}/design`}>Configurar</Link><a className="btn btn-outline" href={publicUrl} target="_blank" rel="noreferrer">Ver como cliente</a></div>
    </header>
    ...
    <section className="local-hub-section"><div><span className="page-eyebrow">GESTIONAR ESTE LOCAL</span><h2>Elige una tarea</h2><p>Todo lo que abras queda restringido a {local.name}.</p></div><div className="local-hub-grid">
      <Link className="local-hub-card is-primary" to={`${base}/forms/${id}/design`}><span className="local-hub-icon">✦</span><div><strong>Configurar</strong><small>Marca, horarios, cupos, campos, medición y publicación.</small></div><b>Editar local →</b></Link>
    ...
    <section className="local-hub-status">
      <div><span className="page-eyebrow">PUBLICACIÓN</span><h2>Identidad y enlace</h2><p className="page-subtitle">El logo y la portada se guardan y se ven en la vista previa pública.</p></div>
      <div className="reservation-metric-grid reservation-metric-grid-four"><div><span>Estado</span><strong>{local.status === 'published' ? 'Publicado' : local.status === 'paused' ? 'Pausado' : 'Borrador'}</strong></div><div><span>Logo</span><strong>{hasLogo ? 'Configurado' : 'Pendiente'}</strong></div><div><span>Portada</span><strong>{hasBackground ? 'Configurada' : 'Pendiente'}</strong></div><div><span>Meta CAPI</span><strong>{local.metaCapiEnabled ? 'Activada en este local' : 'Desactivada'}</strong></div></div>
