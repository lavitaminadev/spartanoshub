...
export function ReservationsPage({ clientView = false }: { clientView?: boolean }) {
  ...
  const resetFilters = (patch: Partial<typeof filters>) => { setFilters((current) => ({ ...current, ...patch })); setPage(1); };
    ...
    return <div className="page reservation-module">
      ...
      {tab === 'forms' && <section>
      <div className="reservation-section-head"><div><span className="page-eyebrow">CAPTACIÓN</span><h1>Formularios de reserva</h1></div><div className="reservation-actions"><p>{visibleForms.length} de {reservationClientForms.length} flujos visibles</p>{!clientView && <div className="reservation-flow-actions"><button className="btn reservation-cta" onClick={() => openCreateFlow('appointment')}>Nuevo formulario de reserva</button><Link className="btn btn-outline btn-sm" to="/surveys">Ir a Encuestas</Link></div>}</div></div>
      ...
      {visibleForms.length === 0 ? <div className="reservation-empty"><strong>Crea tu primer formulario de reserva</strong><p>Las encuestas se crean y publican desde su propio módulo.</p>{!clientView && <div className="reservation-flow-actions"><button className="btn btn-primary" onClick={() => openCreateFlow('appointment')}>Crear formulario de reserva</button><Link className="btn btn-outline" to="/surveys">Ir a Encuestas</Link></div>}</div> : <div className="reservation-form-grid">
    ...
    <Modal open={createOpen} onClose={closeCreateFlow} title="Nuevo formulario de reserva">
      <form className="modal-form reservation-create-wizard" onSubmit={(event) => { event.preventDefault(); if (createStep < 2) { if (createStepReady[createStep]) setCreateStep((current) => Math.min(2, current + 1)); return; } createMutation.mutate(); }}>
        ...
        {createStep === 0 && <>
          <p className="page-subtitle">Define la empresa, el tipo de reserva y el nombre base del formulario.</p>
          ...
          <label>Nombre del formulario de reserva<input className="input" required value={formData.name} onChange={(event) => setFormData({ ...formData, name: event.target.value })} placeholder="Ej. Reserva terraza noche" /></label>
