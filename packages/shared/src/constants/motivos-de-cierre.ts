...
const POR_CLAVE = new Map(MOTIVOS_DE_CIERRE.map((motivo) => [motivo.clave, motivo.nombre]));

export function esMotivoDeCierre(valor: string): valor is MotivoDeCierre {
  return POR_CLAVE.has(valor as MotivoDeCierre);
}

/** El nombre que se muestra, o la clave cruda si llega una que ya no está en el catálogo. */
export function nombreDelMotivo(clave?: string | null): string {
  if (!clave) return '';
  return POR_CLAVE.get(clave as MotivoDeCierre) ?? clave;
}
