...
function validateOrganizationSettingValue(definition, value) {
    ...
    if (definition.valueType === 'boolean') {
        if (typeof value !== 'boolean')
            throw new Error('debe ser verdadero o falso');
        return value;
    }
    if (definition.valueType === 'number') {
        ...
        if (definition.min !== undefined && value < definition.min)
            throw new Error(`no puede ser menor que ${definition.min}`);
        if (definition.max !== undefined && value > definition.max)
            throw new Error(`no puede ser mayor que ${definition.max}`);
        return value;
    }
    ...
    if (normalized.length > 255)
        throw new Error('no puede superar 255 caracteres');
