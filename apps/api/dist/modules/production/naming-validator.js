...
const naming_validation_result_1 = require("./naming-validation-result");
const NAMING_PATTERN = /^([A-Z0-9]+)_([A-Z-]+)_([A-Z0-9-]+)_v(\d+)_(FINAL|BORRADOR|REVISION)$/i;
function validate(fileName, clientCode) {
    const errors = [];
    if (/\s/.test(fileName)) {
    ...
    if (!match) {
        ...
        return new naming_validation_result_1.NamingValidationResult(fileName, false, errors);
    }
    return new naming_validation_result_1.NamingValidationResult(fileName, errors.length === 0, errors);
}
function isValid(fileName, clientCode) {
