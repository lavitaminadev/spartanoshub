...
const LEVEL_COLORS: Record<PermissionLevel, string> = { none: '#889991', view: '#1f6fb2', edit: '#9a5a00', manage: '#1f7a46' };
...
export function AdminPage() {
  ...
  const [tab, setTab] = useState<'permisos' | 'excepciones' | 'consentimiento'>('permisos');
  ...
  return <div className="page governance-page">
    ...
    <nav className="governance-tabs">
      ...
      <button className={tab === 'excepciones' ? 'active' : ''} onClick={() => { setTab('excepciones'); setFeedback(null); }}><span>02</span><strong>Accesos por persona</strong><small>Excepciones y accesos temporales</small></button>
      <button className={tab === 'consentimiento' ? 'active' : ''} onClick={() => { setTab('consentimiento'); setFeedback(null); }}><span>03</span><strong>Consentimiento</strong><small>Versiones, aceptaciÃ³n y acceso</small></button>
    </nav>
    ...
    {tab === 'excepciones' && <section>
      ...
      <div className="exception-summary">{activeExCount > 0 || expiredExCount > 0 ? <div className="section-metrics"><span><strong>{activeExCount}</strong><small>Activas</small></span><span><strong>{expiredExCount}</strong><small>Vencidas</small></span></div> : null}</div>
      ...
      {visibleExceptions.length === 0
        ? <EmptyState icon="ðŸ”‘" title="Sin excepciones" description={exceptions.length === 0 ? 'No hay excepciones de acceso. Crea una para otorgar acceso temporal.' : 'Ninguna excepciÃ³n coincide con el filtro.'} action={exceptions.length === 0 ? <button className="btn btn-primary" onClick={() => setExceptionOpen(true)}>Crear primera excepciÃ³n</button> : undefined} />
        : <div className="table-wrapper"><table className="data-table"><thead><tr><th>Persona</th><th>Cargo</th><th>MÃ³dulo</th><th>Nivel</th><th>Vencimiento</th><th>Motivo</th><th></th></tr></thead><tbody>{visibleExceptions.map((e) => <tr key={e.id} className={e.status === 'expired' ? 'row-muted' : ''}><td><strong>{e.userName}</strong></td><td>{e.userRole ? (ROLE_LABELS[e.userRole] ?? e.userRole) : 'â€”'}</td><td>{e.module}</td><td><span className={`level-pill level-${e.level}`} style={{ backgroundColor: `${LEVEL_COLORS[e.level as PermissionLevel] ?? '#889991'}18`, color: LEVEL_COLORS[e.level as PermissionLevel] ?? '#889991', borderColor: LEVEL_COLORS[e.level as PermissionLevel] ?? '#889991' }}>{LEVEL_LABELS[e.level as PermissionLevel] ?? e.level}</span></td><td>{e.expiresAt ? new Date(e.expiresAt).toLocaleDateString('es-CL') : 'Sin vencimiento'}<br /><small><StatusBadge status={e.status} /></small></td><td>{e.reason || 'â€”'}</td><td>{e.status === 'active' ? <button className="btn btn-outline btn-sm" onClick={() => setRevokeId(e.id)}>Revocar</button> : null}</td></tr>)}</tbody></table></div>}
    </section>}
    ...
    <Modal open={exceptionOpen} onClose={() => setExceptionOpen(false)} title="Nueva excepciÃ³n de acceso">
      <form className="modal-form" onSubmit={(e) => { e.preventDefault(); excCreateMutation.mutate(exceptionDraft); }}>
        ...
        <label>MÃ³dulo<select className="input" required value={exceptionDraft.module} onChange={(e) => setExceptionDraft({ ...exceptionDraft, module: e.target.value })}><option value="">Seleccionar mÃ³dulo</option>{MODULE_CATALOG.filter((m) => isModuleLifecycleVisible(m.lifecycle)).map((m) => <option key={m.key} value={m.key}>{m.key}</option>)}</select></label>
    ...
    <ConfirmDialog open={!!revokeId} title="Revocar excepciÃ³n" description="La persona volverÃ¡ al nivel de acceso que su cargo define. Esta acciÃ³n se registra en auditorÃ­a." confirmLabel="Revocar" pending={excRevokeMutation.isPending} onClose={() => setRevokeId(null)} onConfirm={() => { if (revokeId) { excRevokeMutation.mutate(revokeId, { onSuccess: () => setRevokeId(null) }); } }} />
  </div>;
}
