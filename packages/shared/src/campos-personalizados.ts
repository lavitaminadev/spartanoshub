...
export const TIPOS_DE_CAMPO: ReadonlyArray<{ value: CustomFieldType; label: string }> = [
...
export function problemaDeClave(clave: string): string | null {
  if (!/^[a-z][a-z0-9_]{1,39}$/.test(clave)) return 'Usa minúsculas, números y guion bajo, empezando por una letra (2 a 40 caracteres)';
  if (CLAVES_RESERVADAS.has(clave)) return `«${clave}» ya es un dato propio del CRM`;
  return null;
}
...
export function claveDesdeEtiqueta(etiqueta: string): string {
  ...
  return CLAVES_RESERVADAS.has(conLetra) ? `${conLetra}_propio`.slice(0, 40) : conLetra;
}
 ...
 */
export function normalizarValor(def: Pick<CustomFieldDefinition, 'type' | 'options' | 'label'>, valor: unknown): { ok: true; valor: CustomFieldValue } | { ok: false; error: string } {
  switch (def.type) {
    case 'text':
    case 'long_text': {
      const texto = String(valor).trim();
      const limite = def.type === 'text' ? LARGO_TEXTO : LARGO_TEXTO_LARGO;
      if (texto.length > limite) return { ok: false, error: `«${def.label}» admite hasta ${limite} caracteres` };
      return { ok: true, valor: texto };
    }
    case 'number': {
      const numero = typeof valor === 'number' ? valor : Number(String(valor).replace(/\s/g, '').replace(',', '.'));
      if (!Number.isFinite(numero)) return { ok: false, error: `«${def.label}» tiene que ser un número` };
    ...
    case 'boolean': {
      ...
      if (['true', 'si', 'sí', '1', 'yes'].includes(texto)) return { ok: true, valor: true };
      if (['false', 'no', '0'].includes(texto)) return { ok: true, valor: false };
      return { ok: false, error: `«${def.label}» tiene que ser sí o no` };
    }
    case 'select': {
      const texto = String(valor).trim();
      if (!(def.options ?? []).includes(texto)) return { ok: false, error: `«${texto}» no es una opción de «${def.label}»` };
      return { ok: true, valor: texto };
    }
    case 'multi_select': {
      ...
      if (fuera.length) return { ok: false, error: `${fuera.map((f) => `«${f}»`).join(', ')} no ${fuera.length === 1 ? 'es opción' : 'son opciones'} de «${def.label}»` };
...
export function validarCamposPersonalizados(
  definiciones: CustomFieldDefinition[],
  ...
  const valores: CustomFieldValues = { ...(previos ?? {}) };
  const errores: string[] = [];

  for (const [clave, crudo] of Object.entries(nuevos ?? {})) {
    const def = porClave.get(clave);
    if (!def) { errores.push(`El campo «${clave}» no existe`); continue; }
    // Archivado: lo guardado se conserva, pero no se escribe encima.
    if (def.archivedAt) { errores.push(`«${def.label}» está archivado y no se puede editar`); continue; }
    if (vacio(crudo)) { delete valores[clave]; continue; }
  ...
  if (opciones.exigirObligatorios) {
    for (const def of definiciones) {
      if (def.required && !def.archivedAt && vacio(valores[def.key])) errores.push(`Falta completar «${def.label}»`);
    }
  }
 ...
 */
export function valoresQueSeRomperian(
  def: Pick<CustomFieldDefinition, 'key' | 'type' | 'options' | 'label'>,
  tipoNuevo: CustomFieldType,
  opcionesNuevas: string[] | null | undefined,
  guardados: Array<CustomFieldValues | null | undefined>,
): number {
  let rotos = 0;
