...
export function ReservationLocalHubPage() {
  ...
  return <main className="reservation-local-hub">
    ...
    <section className="local-hub-section"><div><span className="page-eyebrow">GESTIONAR ESTE LOCAL</span><h2>Elige una tarea</h2><p>Todo lo que abras queda restringido a {local.name}.</p></div><div className="local-hub-grid">
      <Link className="local-hub-card is-primary" to={`${base}/forms/${id}/design?section=esencial`}><span className="local-hub-icon">1</span><div><strong>Configuración esencial</strong><small>Horarios, cupos, campos, diseño y publicación en un único recorrido.</small></div><b>Configurar y publicar</b></Link>
      <Link className="local-hub-card" to={`${base}?tab=bookings&clientId=${encodeURIComponent(local.clientId)}&formId=${encodeURIComponent(id)}`}><span className="local-hub-icon">2</span><div><strong>Reservas</strong><small>Lista masiva, filtros, exportación y detalle.</small></div><b>Ver reservas</b></Link>
      <Link className="local-hub-card" to={`${base}/agenda?clientId=${encodeURIComponent(local.clientId)}&formId=${encodeURIComponent(id)}`}><span className="local-hub-icon">3</span><div><strong>Agenda y grupos</strong><small>Turno, bloqueos y solicitudes de evento.</small></div><b>Abrir agenda</b></Link>
      <Link className="local-hub-card" to={`${base}/calendar?clientId=${encodeURIComponent(local.clientId)}&formId=${encodeURIComponent(id)}`}><span className="local-hub-icon">4</span><div><strong>Disponibilidad</strong><small>Ocupación mensual de este local, sin mezclar sedes.</small></div><b>Ver calendario</b></Link>
      <Link className="local-hub-card" to={`${base}?tab=coupons&clientId=${encodeURIComponent(local.clientId)}&formId=${encodeURIComponent(id)}`}><span className="local-hub-icon">5</span><div><strong>Cupones</strong><small>Vigencia, usos y reservas que lo aplicaron.</small></div><b>Gestionar cupones</b></Link>
      <Link className="local-hub-card" to={`${base}/forms/${id}/advanced`}><span className="local-hub-icon">6</span><div><strong>Ajustes especiales</strong><small>Zonas, alergias, niños, consentimientos y cierres.</small></div><b>Abrir ajustes</b></Link>
    </div></section>
    ...
    <section className="local-hub-status">
      ...
      <div className="reservation-metric-grid reservation-metric-grid-four"><div><span>Estado</span><strong>{local.status === 'published' ? 'Publicado' : local.status === 'paused' ? 'Pausado' : 'Borrador'}</strong></div><Link to={`${base}/forms/${id}/design?section=diseno`}><span>Logo</span><strong>{hasLogo ? 'Cambiar logo' : 'Agregar logo'}</strong></Link><Link to={`${base}/forms/${id}/design?section=diseno`}><span>Portada</span><strong>{hasBackground ? 'Cambiar portada' : 'Agregar portada'}</strong></Link><Link to={`${base}/forms/${id}/design?section=medicion`}><span>Medición</span><strong>{local.metaCapiEnabled || local.ga4MeasurementId ? 'Revisar medición' : 'Configurar si la necesitas'}</strong></Link></div>
