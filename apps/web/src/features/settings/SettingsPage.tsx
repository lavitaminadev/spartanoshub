...
const MANAGED_FEATURES = [
  ['direction', 'Dirección'], ['production', 'Producción'], ['audiovisual', 'Audiovisual'],
  ['reports', 'Reportes'], ['meetings', 'Reuniones'], ['documents', 'Documentos'],
] as const;
...
const GROUP_COPY: Record<OrganizationSetting['category'], { kicker: string; title: string; description: string }> = {
  ...
  production: { kicker: 'Trazabilidad', title: 'Reglas de producción', description: 'Controla los puntos del flujo donde Espartanos debe alertar o registrar un cobro extra.' },
...
export function SettingsPage() {
  ...
  const featuresQuery = useQuery<{ features: Record<string, boolean> }>({
    queryKey: ['organization-features'],
    queryFn: () => api.get('/organizations/features'),
    enabled: user?.role === 'admin' || user?.role === 'dev',
  });
  const featureMutation = useMutation({
    mutationFn: ({ key, enabled }: { key: string; enabled: boolean }) => api.put<{ features: Record<string, boolean> }>('/organizations/features', { features: { [key]: enabled } }),
    onSuccess: async (result) => {
      queryClient.setQueryData(['organization-features'], result);
      await refreshProfile();
      await queryClient.invalidateQueries({ queryKey: ['role-preview'] });
      setFeedback({ tone: 'success', text: 'Módulo actualizado para la organización.' });
    },
        ...
        <div className="settings-identity-grid">
          ...
          <section className="settings-form-card organization-card">
            ...
            <form onSubmit={(event) => { event.preventDefault(); organizationMutation.mutate(); }}>
              ...
              <div className="organization-preview"><span>Vista previa onboarding</span><div>{orgLogo ? <img src={orgLogo} alt="Logo" style={{ maxWidth: 80, maxHeight: 40, marginBottom: 8 }} /> : <b>{orgName?.charAt(0) || 'V'}</b>}<p><strong>{orgName || 'Espartanos'}</strong><small>{orgWelcome || 'Bienvenido al espacio operativo Espartanos.'}</small></p></div></div>
      ...
      {activeTab === 'access' && (
        <div className="settings-access-grid">
          {(user?.role === 'admin' || user?.role === 'dev') && (
            <section className="settings-form-card" style={{ gridColumn: '1 / -1' }}>
              <header><span>Control de plataforma</span><h2>Módulos de la organización</h2><p>Activa o desactiva módulos sin volver a desplegar. El cambio afecta a toda la organización.</p></header>
              <div className="settings-access-grid">
                {MANAGED_FEATURES.map(([key, label]) => {
                  const enabled = Boolean(featuresQuery.data?.features?.[key]);
                  return <label key={key} className="central-switch"><input type="checkbox" checked={enabled} disabled={featuresQuery.isLoading || featureMutation.isPending} onChange={(event) => featureMutation.mutate({ key, enabled: event.target.checked })} /><span aria-hidden="true" /><strong>{label}: {enabled ? 'Activado' : 'Desactivado'}</strong></label>;
                })}
              </div>
            </section>
          )}
