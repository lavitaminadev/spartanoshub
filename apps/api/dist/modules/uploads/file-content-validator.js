...
function contentMatches(mimeType, buffer) {
    ...
    if (mimeType === 'image/avif')
        return isIsoMedia(buffer) && ['avif', 'avis'].includes(buffer.subarray(8, 12).toString('ascii'));
    ...
    if (LEGACY_OFFICE_MIMES.has(mimeType))
    ...
    if (mimeType === 'audio/mp4' || mimeType === 'video/mp4')
        return isIsoMedia(buffer);
    if (mimeType === 'video/quicktime')
        return isIsoMedia(buffer) && buffer.subarray(8, 12).toString('ascii').trim() === 'qt';
    if (mimeType === 'video/webm')
        return startsWith(buffer, [0x1a, 0x45, 0xdf, 0xa3]);
    return false;
}
function validateFileContent(mimeType, buffer) {
    if (!exports.ALLOWED_MIME_TYPES.has(mimeType))
        throw new common_1.BadRequestException('El tipo de archivo no está permitido');
    if (!contentMatches(mimeType, buffer)) {
        throw new common_1.BadRequestException('El contenido del archivo no coincide con el tipo declarado');
    }
