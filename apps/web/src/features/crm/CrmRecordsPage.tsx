...
export function OpportunitiesPage() {
  ...
  const openEdit = (item?: Opportunity) => { setEditing(item ?? null); setForm(item ? { name: item.name, amount: item.amount == null ? '' : String(item.amount), stage: item.stage, probability: String(item.probability), expectedCloseDate: item.expectedCloseDate?.slice(0, 10) ?? '', nextAction: item.nextAction ?? '', nextActionAt: item.nextActionAt ? new Date(item.nextActionAt).toISOString().slice(0, 16) : '', leadId: item.leadId ?? '', clientId: item.clientId ?? '' } : EMPTY_OPPORTUNITY); setCamposPropios((item?.customFields ?? {}) as ValoresEnEdicion); setOpen(true); };
