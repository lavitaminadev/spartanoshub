...
exports.nombreLegalDelLocal = nombreLegalDelLocal;
exports.politicaDePrivacidadDeEspartanos = politicaDePrivacidadDeEspartanos;
exports.condicionesDeUso = condicionesDeUso;
exports.condicionesDelServicio = condicionesDelServicio;
exports.politicaDeMedicion = politicaDeMedicion;
exports.contratoDeEncargo = contratoDeEncargo;
exports.ejercicioDeDerechos = ejercicioDeDerechos;
exports.politicaDePrivacidadDelLocal = politicaDePrivacidadDelLocal;
exports.documentoDeEspartanos = documentoDeEspartanos;
exports.rutaDocumentoLegal = rutaDocumentoLegal;
...
exports.PLAZO_AVISO_VULNERACION_HORAS = 48;
/** Versión vigente del conjunto de documentos. */
exports.VERSION_DOCUMENTOS_LEGALES = 'legal-2026-09-15b';
exports.VIGENCIA_DOCUMENTOS_LEGALES = '2026-09-15';
const CORREO = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;
/** Lo que falta para que un local pueda publicar algo que recoge datos personales. Vacío si está completo. */
function faltantesDeIdentidadLegal(identidad) {
    const faltan = [];
    if (!identidad.razonSocial?.trim())
        faltan.push('razón social');
    if (!identidad.rut?.trim() || !(0, survey_rules_1.rutValido)(identidad.rut))
        faltan.push('RUT válido');
    if (!identidad.correo?.trim() || !CORREO.test(identidad.correo.trim()))
        faltan.push('correo para consultas de privacidad');
    return faltan;
}
/** Mensaje para quien intenta publicar sin los datos legales del local. */
function mensajeDeIdentidadIncompleta(faltan) {
...
function nombreLegalDelOperador() {
    const { marca, razonSocial, rut } = exports.OPERADOR_ESPARTANOS;
    if (razonSocial)
        return `${razonSocial}${rut ? `, RUT ${rut}` : ''} («${marca}»)`;
    return marca;
}
/** Cómo se nombra al local en los textos. */
function nombreLegalDelLocal(identidad) {
...
function identidadDelOperador() {
    const { correo, sitio, domicilio } = exports.OPERADOR_ESPARTANOS;
    return [
...
function politicaDePrivacidadDeEspartanos() {
    const p = exports.PLAZOS_DE_CONSERVACION;
    const { correo, alojamiento, contactoPrivacidad } = exports.OPERADOR_ESPARTANOS;
    return {
        id: 'privacidad',
        titulo: 'Política de privacidad de Espartanos',
        version: exports.VERSION_DOCUMENTOS_LEGALES,
        vigenteDesde: exports.VIGENCIA_DOCUMENTOS_LEGALES,
        ...
        secciones: [
            ...
            },
            {
                ...
                cierre: [
                    `Cómo ejercerlos y en qué plazo se responde se explica en «Tus derechos». El plazo es de ${exports.PLAZO_RESPUESTA_DERECHOS_DIAS} días corridos desde que se recibe la solicitud, prorrogable una vez por el mismo período con aviso fundado.`,
                    RECLAMO,
                ],
            },
            {
...
function condicionesDeUso() {
    const { marca, correo } = exports.OPERADOR_ESPARTANOS;
    return {
        id: 'terminos',
...
function condicionesDelServicio() {
    const { marca, correo } = exports.OPERADOR_ESPARTANOS;
    return {
        id: 'servicio',
        ...
        secciones: [
            ...
            { titulo: '4. Transferencia fuera de Chile', parrafos: ['Meta Platforms y Google tratan estos datos en servidores ubicados principalmente en Estados Unidos y la Unión Europea, amparados en sus cláusulas contractuales tipo y garantías. Cada uno es responsable del uso que haga de los datos conforme a su propia política de privacidad.'] },
            {
                ...
                parrafos: [
                    ...
                    `Los identificadores de medición guardados junto a una reserva se anonimizan a los ${exports.PLAZOS_DE_CONSERVACION.medicionMeses} meses. Los datos que Meta o Google ya recibieron se rigen por sus políticas; puedes gestionarlos desde la configuración de privacidad de tu cuenta en esas plataformas.`,
                    'También puedes bloquear o borrar cookies desde la configuración de tu navegador.',
                ],
            },
            { titulo: '6. Contacto', parrafos: [`Para consultas sobre la medición escribe a ${exports.OPERADOR_ESPARTANOS.correo}.`, RECLAMO] },
        ],
    };
}
/** Contrato de encargo de tratamiento que acepta cada empresa al usar Reservas o Encuestas. */
function contratoDeEncargo() {
    const p = exports.PLAZOS_DE_CONSERVACION;
    const { marca, correo, alojamiento } = exports.OPERADOR_ESPARTANOS;
    return {
        id: 'encargo',
...
function ejercicioDeDerechos() {
    const { correo, contactoPrivacidad } = exports.OPERADOR_ESPARTANOS;
    return {
        id: 'derechos',
        ...
        secciones: [
            ...
            },
            {
                ...
                parrafos: [],
                lista: [
                    `Respuesta: dentro de ${exports.PLAZO_RESPUESTA_DERECHOS_DIAS} días corridos desde que se recibe la solicitud.`,
                    `Prórroga: una sola vez, por ${exports.PLAZO_RESPUESTA_DERECHOS_DIAS} días corridos más, informándote antes del vencimiento y explicando el motivo.`,
    ...
    return {
        ...
        secciones: [
            ...
            { titulo: '8. Tus derechos', parrafos: [
                    `Puedes ejercer los derechos de ${DERECHOS}, pedir revisión humana de decisiones automatizadas y retirar autorizaciones, sin costo, escribiendo a ${correo}. Respondemos dentro de ${exports.PLAZO_RESPUESTA_DERECHOS_DIAS} días corridos, prorrogables una vez por igual plazo con aviso fundado. El detalle está en «Tus derechos» de ${marca}.`,
                    RECLAMO,
                ] },
...
function documentoDeEspartanos(id) {
    switch (id) {
...
exports.DOCUMENTOS_DE_ESPARTANOS = [
...
function textosDeAceptacionDeReserva(identidad, opciones = {}) {
    const local = nombreLegalDelLocal(identidad);
    const correo = identidad.correo?.trim() || exports.OPERADOR_ESPARTANOS.correo;
 ...
 */
function textoDeDatosSensibles(identidad) {
    const local = nombreLegalDelLocal(identidad);
    const correo = identidad.correo?.trim() || exports.OPERADOR_ESPARTANOS.correo;
...
exports.CAMPOS_DE_VISITA_SENSIBLES = ['accessibilityNeed', 'dietaryNotes'];
const tieneValor = (valor) => (Array.isArray(valor) ? valor.length > 0 : typeof valor === 'string' ? valor.trim() !== '' : valor !== undefined && valor !== null && valor !== false);
/**
 ...
 */
function traeDatosSensibles(campos, respuestas, visita = {}) {
    if (exports.CAMPOS_DE_VISITA_SENSIBLES.some((clave) => tieneValor(visita[clave])))
        return true;
    return campos.some((campo) => campo.sensible && tieneValor(respuestas?.[campo.id]));
}
exports.MENSAJE_FALTA_CONSENTIMIENTO_SENSIBLE = 'Para guardar la información de salud o alimentación que indicaste, marca la casilla que la autoriza. Si prefieres, bórrala y envía sin ella.';
//# sourceMappingURL=documentos-legales.js.map