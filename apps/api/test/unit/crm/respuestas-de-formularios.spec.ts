...
import { repartirRespuestas, type CampoConPreguntas } from '../../../src/modules/crm/fields/respuestas-de-formularios';
...
describe('repartirRespuestas', () => {
  it('lleva la respuesta al campo que declaró esa pregunta', () => {
    const definiciones = [campo({ metaQuestions: ['¿Cuál es tu presupuesto?'] })];
    const { camposPropios, sinCampo } = repartirRespuestas(definiciones, [{ nombre: 'cual_es_tu_presupuesto', valor: '$500.000' }]);
  ...
  it('la clave del campo ya vale como equivalencia, sin configurar nada', () => {
    const { camposPropios } = repartirRespuestas([campo({})], [{ nombre: 'Presupuesto Mensual', valor: '250000' }]);
    expect(camposPropios).toEqual({ presupuesto_mensual: 250000 });
  });

  it('lo que ningún campo reclama se queda para las notas', () => {
    const { camposPropios, sinCampo } = repartirRespuestas([campo({})], [{ nombre: 'cuando_empezar', valor: 'Este mes' }]);
  ...
  it('un valor que no encaja con el tipo no se guarda: iría a romper los filtros', () => {
    const { camposPropios, sinCampo } = repartirRespuestas([campo({})], [{ nombre: 'presupuesto_mensual', valor: 'lo que sea necesario' }]);
    expect(camposPropios).toEqual({});
  ...
  it('una opción se reconoce aunque venga con otra acentuación', () => {
    ...
    const { camposPropios } = repartirRespuestas(definiciones, [{ nombre: 'Contacto preferido', valor: 'whatsapp' }]);
  ...
  it('varias opciones llegan separadas por coma y se quedan sólo las que existen', () => {
    ...
    const { camposPropios } = repartirRespuestas(definiciones, [{ nombre: 'Qué te interesa', valor: 'Meta Ads, Otra cosa, Branding' }]);
  ...
  it('un campo archivado no recibe nada', () => {
    ...
    const { camposPropios, sinCampo } = repartirRespuestas(definiciones, [{ nombre: 'presupuesto', valor: '100000' }]);
    expect(camposPropios).toEqual({});
    expect(sinCampo).toHaveLength(1);
  });
});
