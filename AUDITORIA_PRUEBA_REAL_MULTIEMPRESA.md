...
8. **P0 corregido, pendiente de repetición con portal — Portal y cartera de empresas.** Una sesión real de portal CRM recibió 12 empresas por `GET /clients?limit=100`, en vez de solo la asignada. El portal ya toma empresa exclusivamente de `req.user.clientId` y dejó de consultar el catálogo; dicho catálogo está fuera del alcance inicial y responde 403. La regresión pasó CI y fue desplegada; falta repetir el ingreso de un portal real y probar URL/API manipuladas.
...
18. **P1 — Alcance inicial aún no es mínimo.** En producción, `CRM` y `Reservas` están activos, pero también `dashboard`, `settings`, `users`, `clients`, `reports`, `integrations`, `governance` y `surveys`. Los módulos futuros apagados sí responden bloqueo aun por URL directa. Antes de una prueba con clientes se debe decidir y aplicar el preset de operación mínima, manteniendo las dependencias administrativas estrictamente necesarias.
19. **P2 — Selector de empresas ambiguo con nombres repetidos.** Desarrollo ve dos opciones con el mismo nombre visible. El aislamiento depende de sus identificadores internos, pero una persona no puede distinguirlas al elegir. Las empresas deben tener nombre visible único o el selector debe añadir un código/identificador no sensible.
