...
export function GovernancePage() {
  ...
  if (workflowsQuery.isLoading || podsQuery.isLoading || usersQuery.isLoading || clientsQuery.isLoading) {
  ...
  return <div className="page governance-page">
    <PageHero
      ...
      aside={<div className="page-hero-stats"><span><small>Flujos</small><strong>{workflowsQuery.data?.length ?? 0}</strong></span><span><small>Pods activos</small><strong>{podsQuery.data?.filter((pod) => pod.status === 'active').length ?? 0}</strong></span></div>}
    />
