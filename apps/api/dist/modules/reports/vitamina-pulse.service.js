...
exports.VitaminaPulseService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
let VitaminaPulseService = class VitaminaPulseService {
    constructor(dataSource) {
        this.dataSource = dataSource;
    }
    async getPulse(organizationId, clientId, clientIds) {
        ...
        const [cycleRows, pieceRows, approvalRows, meetingRows, metricRows, reservationRows, clientRows] = await Promise.all([
            this.dataSource.query(`SELECT COUNT(*) total,
         ...
         FROM pieces WHERE organization_id = ?${scope} AND created_at >= DATE_SUB(NOW(), INTERVAL 90 DAY)`, params),
            this.dataSource.query(`SELECT COUNT(*) total, SUM(status = 'pending') pending,
         ...
         FROM approval_requests WHERE organization_id = ?${scope} AND created_at >= DATE_SUB(NOW(), INTERVAL 90 DAY)`, params),
            this.dataSource.query(`SELECT COUNT(*) total, SUM(status = 'completed') completed,
         ...
         FROM meetings WHERE organization_id = ?${scope} AND scheduled_at >= DATE_SUB(NOW(), INTERVAL 45 DAY)`, params),
            this.dataSource.query(`SELECT COUNT(*) total, COALESCE(SUM(leads),0) leads, COALESCE(SUM(conversions),0) conversions,
         ...
         FROM integration_metrics WHERE organization_id = ?${scope} AND metric_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)`, params),
            this.dataSource.query(`SELECT COUNT(*) total, SUM(status = 'attended') attended, SUM(status = 'no_show') no_show,
         ...
         FROM reservations WHERE organization_id = ?${scope} AND starts_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)`, params),
            this.dataSource.query(`SELECT COUNT(*) total, MIN(started_at) oldest_start, SUM(status = 'active') active
        ...
        const ratio = (value, total) => total > 0 ? Math.round(Math.min(value / total, 1) * 100) : null;
        const statusFor = (score) => score == null ? 'no_data' : score >= 80 ? 'healthy' : score >= 55 ? 'attention' : 'blocked';
        ...
        const actions = [];
        if (overduePieces > 0)
            actions.push({ id: 'overdue-pieces', title: `Resolver ${overduePieces} pieza${overduePieces === 1 ? '' : 's'} vencida${overduePieces === 1 ? '' : 's'}`, detail: 'Desbloquea responsables o redefine la fecha antes de comprometer nuevas entregas.', priority: 'high', owner: 'team', route: '/production' });
        if (Number(approvals.overdue || 0) > 0)
            actions.push({ id: 'overdue-approvals', title: 'Responder aprobaciones fuera de plazo', detail: 'La producción está esperando una decisión del cliente.', priority: 'high', owner: 'client', route: clientId ? '/portal/approvals' : '/approvals' });
        else if (Number(approvals.pending || 0) > 0)
            actions.push({ id: 'pending-approvals', title: `${approvals.pending} aprobaciones requieren atención`, detail: 'Revisarlas mantiene el calendario creativo en movimiento.', priority: 'medium', owner: 'client', route: clientId ? '/portal/approvals' : '/approvals' });
        if (planningScore == null || planningScore < 100)
            actions.push({ id: 'planning', title: 'Completar la planificación del ciclo', detail: 'Confirma grilla, prioridades y ventanas de publicación.', priority: planningScore == null ? 'medium' : 'low', owner: 'team', route: clientId ? '/portal/grid' : '/content' });
        if (measurementScore == null)
            actions.push({ id: 'measurement', title: 'Activar medición de resultados', detail: 'Asigna y sincroniza las cuentas publicitarias para conectar trabajo e impacto.', priority: 'medium', owner: 'team', route: '/integrations' });
        if (Number(reservationActivity.no_show || 0) > 0)
            actions.push({ id: 'reservation-no-show', title: `Reducir ${reservationActivity.no_show} inasistencia${Number(reservationActivity.no_show) === 1 ? '' : 's'}`, detail: 'Revisa anticipación, horarios de mayor abandono y recordatorios del local.', priority: 'medium', owner: 'team', route: clientId ? '/portal/reservations' : '/reservations' });
        if (!actions.length)
            actions.push({ id: 'healthy-cycle', title: 'Mantener el ritmo del ciclo', detail: 'La cuenta no presenta bloqueos operativos prioritarios.', priority: 'low', owner: 'team', route: clientId ? '/portal/reports' : '/reports' });
        const timelineParams = [params, params, params].flat();
        const timeline = await this.dataSource.query(`SELECT * FROM (
...
exports.VitaminaPulseService = VitaminaPulseService;
exports.VitaminaPulseService = VitaminaPulseService = __decorate([
