...
export function PulsoEspartano({ compact = false }: { compact?: boolean }) {
  ...
  return <section className={`pulse-shell ${compact ? 'is-compact' : ''}`}>
    ...
    <div className="pulse-overview">
      ...
      <div className="commitment-card"><span>Compromisos abiertos</span>{!compact && <div><strong>{data.commitments.team}</strong><small>Espartanos</small></div>}<div><strong>{data.commitments.client}</strong><small>{compact ? 'Tuyos' : 'Cliente'}</small></div></div>
    </div>
