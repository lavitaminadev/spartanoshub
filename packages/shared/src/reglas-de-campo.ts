 ...
 */
export function campoVisible(campo: CampoConRegla, respuestas: Record<string, unknown>): boolean {
  const regla = campo.mostrarSi;
  if (!regla || !regla.campo) return true;
  ...
  switch (regla.operador) {
    case OPERADORES_DE_CAMPO.RESPONDIDO:
      return !estaVacio(actual);
    case OPERADORES_DE_CAMPO.VACIO:
      return estaVacio(actual);
    case OPERADORES_DE_CAMPO.IGUAL:
 ...
 */
function comparables(valor: unknown): string[] {
  if (Array.isArray(valor)) return valor.map((entrada) => String(entrada).trim().toLowerCase());
 ...
 */
export function camposVisibles<T extends CampoConRegla>(campos: T[], respuestas: Record<string, unknown>): T[] {
  const visibles: T[] = [];
  ...
  for (const campo of campos) {
    ...
    if (huerfana || (!dependeDeUnoOculto && campoVisible(campo, respuestas))) visibles.push(campo);
    else ocultos.add(campo.id);
  }
  return visibles;
}
