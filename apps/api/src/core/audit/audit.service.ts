...
export class AuditService {
  ...
  /**
   * @param options.entityId - Historial de un registro concreto. Es lo que se necesita para
   *   responder "quién cambió esto y desde qué valor", que era imposible mientras no se
   *   pudiera filtrar por entidad.
   */
  async search(organizationId: string, options: { entityType?: string; entityId?: string; action?: string; actorId?: string; limit?: number }) {
    ...
    if (options.entityId) query.andWhere('audit.entity_id = :entityId', { entityId: options.entityId });
