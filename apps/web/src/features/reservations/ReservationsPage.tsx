...
type ReservationEvent = EventoDeReserva;
...
/** Tipo de evento de una solicitud, como lo lee el equipo. */
const TIPO_DE_EVENTO: Record<string, string> = { cumpleanos: 'Cumpleaños', aniversario: 'Aniversario o celebración', empresa: 'Evento de empresa', otro: 'Otro grupo' };
/** «2026-09-08» → «8 sept 2026». */
function fechaLegible(fecha: string): string {
  return /^\d{4}-\d{2}-\d{2}$/.test(fecha) ? new Date(`${fecha}T12:00:00Z`).toLocaleDateString('es-CL', { day: 'numeric', month: 'short', year: 'numeric', timeZone: 'UTC' }) : fecha;
}
...
export function ReservationsPage({ clientView = false }: { clientView?: boolean }) {
  ...
  /** Reserva cuya exportación se abrió desde su tarjeta, sin pasar por el filtro de la lista. */
  ...
  const measurementLabel = 'Schedule / Reserva';
  ...
  const openCreateFlow = (mode: 'appointment' | 'group') => {
    setFormData((current) => ({
      ...current,
      clientId: clientFilter,
      mode,
      name: '',
      calendarSaveEnabled: current.calendarSaveEnabled,
    }));
    setCreateStep(0);
    setCreateOpen(true);
  };
  ...
  /*
   * Encuestas se ofrece sólo a quien lo tiene contratado.
   *
   * Cada empresa contrata sus servicios por separado: el enlace se pintaba siempre, así que una
   * que sólo usa Reservas veía una puerta que no le abre. Con una empresa elegida manda la suya;
   * mirando varias, basta con que alguna lo tenga para que el enlace sirva de algo.
   */
  const ofreceEncuestas = empresaOfrece('surveys', clients, clientFilter || user?.clientId);
  ...
  const query = new URLSearchParams({ page: String(page), pageSize: '20', ...(clientFilter ? { clientId: clientFilter } : {}), ...(search ? { search } : {}), ...(filters.status ? { status: filters.status } : {}), ...(filters.formId ? { formId: filters.formId } : {}), ...(filters.resourceId ? { resourceId: filters.resourceId } : {}), ...dateRange });
  const { data: bookingPage, isFetching: loadingBookings, error: bookingsError, refetch: refetchBookings } = useQuery<ReservationPage>({ queryKey: ['reservations', page, clientFilter, search, filters.status, filters.formId, filters.resourceId, selectedFilterForm?.timezone, filters.from, filters.to], queryFn: () => api.get(`/reservations?${query}`), enabled: tab === 'bookings', placeholderData: (previous) => previous });
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      if (designConfig.calendarSaveEnabled) patch.designConfig = designConfig;
  ...
  const manualMutation = useMutation({
    // Recibe el local ya resuelto: con uno solo, el formulario ni siquiera pregunta.
    mutationFn: (formIdElegido?: string) => {
      const elegido = formIdElegido || manualForm.formId;
      const form = forms.find((f) => f.id === elegido);
      const body = {
        ...manualForm,
        formId: elegido,
  ...
  const { data: gruposData = [], isFetching: cargandoGrupos, error: errorGrupos, refetch: recargarGrupos } = useQuery<GroupRequest[]>({
    ...
    queryFn: () => api.get(`/reservations/group-requests?${new URLSearchParams({ ...(filters.formId ? { formId: filters.formId } : {}), ...(clientFilter ? { clientId: clientFilter } : {}) })}`),
    enabled: tab === 'groups',
  });
  ...
  const filteredCoupons = coupons.filter((coupon) => {
    ...
    const appliesToLocal = !filters.formId || !coupon.formIds?.length || coupon.formIds.includes(filters.formId);
    return appliesToLocal && (!needle || coupon.code.toLocaleLowerCase('es').includes(needle));
  });
  ...
  const formPath = (id: string) => clientView ? `/portal/reservations/locals/${id}` : `/reservations/locals/${id}`;
  /** Raiz del modulo segun quien mira: la agencia por su panel, la empresa por su portal. */
  const base = clientView ? '/portal/reservations' : '/reservations';
  ...
  const reservationClientForms = clientForms.filter((form) => !isSurveyMode(form.mode));
  const visibleForms = reservationClientForms.filter((form) => {
  ...
  const resetFilters = (patch: Partial<typeof filters>) => { setFilters((current) => ({ ...current, ...patch })); setPage(1); };
    ...
    return <div className="page reservation-module">
      {clientView && <nav className="reservation-tabs" aria-label="Secciones de reservas">
        {(clientView
          ? ([['forms', 'Mis páginas de reserva'], ['bookings', 'Todas las reservas'], ['groups', 'Grupos y eventos'], ['metrics', 'Resultados']] as const)
      ...
      {visibleForms.length === 0 ? <div className="reservation-empty"><strong>Activa Reservas para tu primer local</strong><p>Las encuestas se crean y publican desde su propio módulo.</p>{!clientView && <div className="reservation-flow-actions"><button className="btn btn-primary" onClick={() => openCreateFlow('appointment')}>Agregar la primera reserva</button>{ofreceEncuestas && <Link className="btn btn-outline" to="/surveys">Ir a Encuestas</Link>}</div>}</div> : <div className="reservation-form-grid">
        {visibleForms.map((form) => <article className="reservation-form-card" key={form.id}>
          ...
          <button type="button" className="btn btn-outline btn-sm" onClick={() => setAjustesDe(form)}>Ajustes del día</button>
            <div className="form-card-actions">{isSurveyMode(form.mode) ? <Link className="btn btn-primary btn-sm" to="/surveys">Abrir encuesta</Link> : <><Link className="btn btn-primary btn-sm" to={`${base}/agenda?clientId=${encodeURIComponent(form.clientId)}&formId=${encodeURIComponent(form.id)}`}>Agenda</Link><Link className="btn btn-outline btn-sm" to={`${base}/forms/${form.id}`}>Configurar</Link><Link className="btn btn-outline btn-sm" to={`${base}/forms/${form.id}/design?section=disponibilidad`}>Bloquear día</Link><Link className="btn btn-outline btn-sm" to={`${base}/forms/${form.id}/design?section=disponibilidad#zonas`}>Zonas y sectores</Link><Link className="btn btn-outline btn-sm" to={`${base}/forms/${form.id}/design?section=diseno`}>Textos y diseño</Link></>}{safeUrl(formPublicUrl(form)) ? <button type="button" className="btn btn-outline btn-sm" onClick={() => setCompartirForm(form)}>Compartir</button> : null}{puedeAccion(useAuth.getState().user, 'reservations.exportar') && <button className="btn btn-outline btn-sm" onClick={() => { setExportFormId(form.id); setExportModalOpen(true); }}>Exportar</button>}{!clientView && form.status !== 'draft' && <button className="btn btn-outline btn-sm" disabled={updateFormMutation.isPending} onClick={() => form.status === 'paused' ? updateFormMutation.mutate({ id: form.id, status: 'published' }) : setConfirmFormAction({ id: form.id, action: 'pause' })}>{updateFormMutation.isPending ? 'Procesando...' : form.status === 'paused' ? 'Reanudar' : 'Pausar'}</button>}</div>
          </article>)}
        </div>}
      </section>}

      {tab === 'bookings' && <section>
      <div className="reservation-section-head"><div><span className="page-eyebrow">OPERACIÓN DE LA RESERVA</span><h1>Todas las reservas</h1></div><div className="reservation-actions"><button className={`btn btn-primary btn-sm ${clientView ? '' : 'duplica-menu'}`} onClick={() => setManualOpen(true)}>Anotar reserva</button>{puedeAccion(useAuth.getState().user, 'reservations.exportar') && <button className="btn btn-outline btn-sm" onClick={() => setExportModalOpen(true)}>Exportar datos</button>}</div></div>
    ...
    {tab === 'groups' && <section>
      ...
      {/*
        * Las solicitudes de todas las reservas llegaban a la misma lista sin forma de acotarla,
        ...
        : <div className="reservation-request-list">{grupos.map((request) => {
          ...
          return <article key={request.id} id={`solicitud-${request.id}`} className={`reservation-request-card${solicitudResaltada === request.id ? ' is-resaltada' : ''}`}>
            <div>
              ...
              <small>{local?.name || 'Reserva no disponible'} · {request.guestPhone || 'Sin teléfono'}{request.guestEmail ? ` · ${request.guestEmail}` : ''}</small>
              ...
              <details open><summary>Lo que pidió</summary><dl className="success-summary">
                <dt>Fecha solicitada</dt><dd>{request.preferredDate ? fechaLegible(request.preferredDate) : 'Por acordar'} {request.preferredTime || ''}{request.preferredDate && local && request.preferredDate < plainDateInZone(new Date(), local.timezone) ? <small className="error-text"> · ya pasó</small> : null}</dd>
            ...
            <div>
              ...
              {(() => { const wsp = whatsappDeSolicitud(request.guestPhone, request.guestName, request.partySize, { monto: request.quoteAmount, detalle: request.quoteMessage }); return wsp && !['converted', 'closed'].includes(request.status) ? <a className="btn btn-outline btn-sm" href={wsp} target="_blank" rel="noopener noreferrer">Responder por WhatsApp</a> : null; })()}
              ...
              {convirtiendo?.id === request.id && <form className="grupo-a-reserva" onSubmit={(event) => { event.preventDefault(); if (!local) return; convertirGrupo.mutate({ id: request.id, startsAt: localInputToUtc(convirtiendo.fecha, local.timezone), resourceId: convirtiendo.zona || undefined }); }}>
                ...
                {convertirGrupo.error && <><small className="error-text">{convertirGrupo.error.message}</small>
                  <small>Puedes elegir una hora dentro de lo publicado o, si el evento se acordó así, crearla igual.</small>
    ...
    <Modal open={createOpen} onClose={closeCreateFlow} title="Agregar una reserva">
      <form className="modal-form reservation-create-wizard" onSubmit={(event) => { event.preventDefault(); if (createStep < 2) { if (createStepReady[createStep]) setCreateStep((current) => Math.min(2, current + 1)); return; } createMutation.mutate(); }}>
        ...
        {createStep === 0 && <>
          <p className="page-subtitle">Elige la empresa: la primera reserva toma su nombre y puedes cambiarlo. Su página permite reservar con hora y también pedir eventos o grupos.</p>
          ...
          <label>Nombre de la reserva<input className="input" required value={formData.name} onChange={(event) => setFormData({ ...formData, name: event.target.value })} placeholder="Ej. Casa Costanera - Providencia" /></label>
        </>}
        {createStep === 1 && <>
          ...
          <label className="toggle-row"><input type="checkbox" checked={formData.calendarSaveEnabled} onChange={(event) => setFormData({ ...formData, calendarSaveEnabled: event.target.checked })} /> Al finalizar, ofrecer guardar en calendario iPhone/Android</label>
        </>}
        {createStep === 2 && <div className="reservation-create-summary">
          <p className="page-subtitle">Revisa la base del formulario antes de abrir el constructor.</p>
          ...
          <ul className="reservation-create-checklist">
            <li>Crearás un formulario de reserva listo para ajustar campos, horarios y publicación.</li>
            <li>{formData.calendarSaveEnabled ? 'La reserva ofrecerá guardar en calendario al finalizar.' : 'La reserva no ofrecerá guardar en calendario por ahora.'}</li>
        ...
        <div className="modal-actions">
          ...
          <button className="btn btn-primary" disabled={createMutation.isPending || (createStep < 2 && !createStepReady[createStep])}>{createMutation.isPending ? 'Creando...' : createStep < 2 ? 'Continuar' : 'Crear y editar formulario'}</button>
        </div>
      </form>
    </Modal>
    ...
    <PanelCompartir
      ...
      pie={compartirForm?.ga4MeasurementId || compartirForm?.metaCapiEnabled
        ? 'La medición de esta reserva recibe cada alta con su canal y campaña.'
