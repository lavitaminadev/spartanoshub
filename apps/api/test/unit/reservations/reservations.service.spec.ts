...
describe('ReservationsService — portada operativa', () => {
  let service: ReservationsService;
  beforeEach(() => {
    vi.clearAllMocks();
    dataSource.query.mockReset();
  ...
  function upcomingReturning(items: unknown[]) {
    const qb: Record<string, unknown> = {};
    for (const method of ['select', 'where', 'andWhere', 'orderBy', 'take']) qb[method] = vi.fn().mockReturnValue(qb);
    qb.getMany = vi.fn().mockResolvedValue(items);
    reservations.createQueryBuilder.mockReturnValue(qb);
    return qb;
  }

  it('delimita el dia en la zona del cliente y no en la del servidor', async () => {
    // Con la base en UTC, un "hoy" calculado a la chilena y otro a la inglesa se desfasan justo
    // en la franja de cena: la reserva de las 21:00 caeria en el dia siguiente.
    forms.findOne.mockResolvedValue({ id: 'form-1', timezone: 'America/Santiago' });
    dataSource.query.mockResolvedValue([{ total: 4, attended: 2, pending: 1, noShow: 0 }]);
    upcomingReturning([]);

    const result = await service.operationalHome('org-1', 'client-1');

    expect(result.timezone).toBe('America/Santiago');
    const [sql, params] = dataSource.query.mock.calls[0];
    expect(sql).toContain('starts_at >= ? AND starts_at < ?');
    // Los limites del dia llegan como instantes, ya resueltos en la zona del cliente.
    expect(params[1]).toBeInstanceOf(Date);
    expect(params[2]).toBeInstanceOf(Date);
    expect((params[2] as Date).getTime()).toBeGreaterThan((params[1] as Date).getTime());
  });

  it('la ocupacion queda sin calcular cuando el cliente no tiene tope diario', async () => {
    // Devolver 0% sugeriria que el local esta vacio; no tener tope es otra cosa.
    forms.findOne.mockResolvedValue({ id: 'form-1', timezone: 'America/Santiago' });
    dataSource.query.mockResolvedValue([{ total: 7, attended: 0, pending: 7, noShow: 0 }]);
    upcomingReturning([]);

    const result = await service.operationalHome('org-1');

    expect(result.today.occupancyPct).toBeNull();
    expect(result.today.total).toBe(7);
  });

  it('sin cuentas alcanzables no devuelve reservas de nadie', async () => {
    dataSource.query.mockResolvedValue([{ total: 0, attended: 0, pending: 0, noShow: 0 }]);
    const qb = upcomingReturning([]);

    const result = await service.operationalHome('org-1', undefined, []);

    expect(result.upcoming).toEqual([]);
    // El alcance vacio filtra por una condicion imposible en vez de omitir el filtro.
    expect(qb.andWhere).toHaveBeenCalledWith('1 = 0', { clientIds: [] });
  });
});
